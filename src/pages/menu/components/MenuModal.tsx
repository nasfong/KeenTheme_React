...
const RoleModal: React.FC<Props> = ({ modal, handleCloseModal, dataInput, updateQuery }) => {

  const [addRole] = useCreateMenu()
  const [updateRole] = useUpdateMenu()

  const [getAllMenuParents, { data: menuParentDropdown, updateQuery: updateQueryMenuParent }] = useQueryMenuParent()
        ...
        updateRole({
          ...
          .then((res) => {
            updateQuery(({ getAllMenus }) => ({
              getAllMenus: getAllMenus.map(data => data.id === res.data?.updateMenu?.id ? res.data?.updateMenu : data)
            }))
            handleClose()
          })
