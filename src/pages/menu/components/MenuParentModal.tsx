...
const MenuParentModal: React.FC<Props> = ({ modal, handleCloseModal, data, updateQuery }) => {
  ...
  const formik = useFormik<MenuParentInput>({
    ...
    onSubmit: (values, { setSubmitting }) => {
        ...
        updateMenuParent({
          ...
          .then((res) => {
            updateQuery(({ getAllMenuParents }) => ({
              getAllMenuParents: getAllMenuParents.map(data => data.id === res.data?.updateMenuParent?.id ? res.data?.updateMenuParent : data)
            }))
            // handleClose()
          })
      ...
      addMenuParent({ variables: { input: values } })
        .then((res) => {
          updateQuery(({ getAllMenuParents }) => ({
            getAllMenuParents: res.data?.createMenuParent ? [...getAllMenuParents, res.data?.createMenuParent] : getAllMenuParents
          }))
          // handleClose()
        })
        .catch((err) => console.log(err))
  ...
  return (
    <>
      <Modal show={modal} onHide={handleClose} size='sm' centered className="bg-dark p-2 text-white bg-opacity-50">
        <Modal.Header>
        ...
        <Form onSubmit={formik.handleSubmit}>
          <Modal.Body>
            <Form.Group as={Row} className='mb-5'>
              <Form.Label column sm='3' className='form-label text-nowrap required'>Name</Form.Label>
              <Col sm>
                <InputV
                  name='name'
                  formik={formik}
