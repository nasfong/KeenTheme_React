  ...
  const formik = useFormik<PermissionInput>({
    ...
    onSubmit: (values, { setSubmitting }) => {
        ...
        updatePermission({
          ...
          .then((res) => {
            updateQuery(({ getAllPermissions }) => ({
              getAllPermissions: getAllPermissions.map(data => data.id === res.data?.updatePermission?.id ? res.data?.updatePermission : data)
            }))
            handleClose()
          })
      ...
      addPermission({ variables: { input: values } })
        .then((res) => {
          updateQuery(({ getAllPermissions }) => ({
            getAllPermissions: res.data?.createPermission ? [...getAllPermissions, res.data.createPermission] : getAllPermissions
          }))
          handleClose()
        })
        .catch((err) => console.log(err))
